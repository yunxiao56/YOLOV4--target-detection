...
class Detect:
    ...
    #     data = img.ctypes.data_as(darknet.POINTER(darknet.c_ubyte))
    ...
    def predict_image(self, image, thresh=0.25, is_show=True, save_path='',imagename = ''):
        '''
        ...
        detections = darknet.detect_image(self.netMain, self.metaMain, darknet_image, self.names, thresh=thresh)

        if is_show:
            imagename = os.path.splitext(imagename)[0]

            f = open("./input/detection-results/" + imagename + ".txt", "w")
            for detection in detections:
                ...
                xyxy = np.array([left,top,right,bottom])
                # label_conf = format((label),{conf:.2f})
                f.write("%s %s %s %s %s %s\n" % (
                label,conf, str(left), str(top), str(right), str(bottom)))


                index = self.names.index(label)
                # label = '{} {:.2f}'.format(label,conf)
                self._plot_one_box(xyxy, rgb_img, self.colors[index], label)
            bgr_img = rgb_img[..., ::-1]
            f.close()
            # src_img = cv2.resize(bgr_img, (int(width), int(height)))
    ...
    def _plot_one_box(self, xyxy, img_rgb, color, label):
        ...
        cv2.putText(img, label, text_pos, cv2.FONT_HERSHEY_SIMPLEX, thickness / 3, [225, 255, 255], thickness=thickness,
...
if __name__ == '__main__':
    ...
    detect = Detect(metaPath=r'./cfg/target.data',
                    configPath=r'./radarData_xc/yolov4-target.cfg',
                    # configPath=r'./cfg/yolov4-target.cfg',
                    weightPath=r'./backup/weights/yolov4_xc_416_color/yolov4-target_2000.weights',
                    # weightPath=r'./backup/yolov3_final.weights',
                    namesPath=r'./cfg/target.names',
                    gpu_id=0)
    # coco权重
    ...
    paths = "/home/admin/PycharmProjects/yolov4-darknet-master/darknet-master/radarData_xc/VOCdevkit/test/JPEGImages/level_6"  # 测试图片的路径
    savepath = r'./radarData_xc/VOCdevkit/testResults_yolo4_2000' # VOCdevkit VOCcom
    ...
    for filename in filenames:
        picpath = os.path.join(paths,filename)
        image = cv2.imread(picpath)
        savpicpath = os.path.join(savepath,filename)
        detect.predict_image(image, save_path=savpicpath,imagename = filename)
